    ...
    <div className="relative flex min-h-screen overflow-hidden bg-surface">
      ...
      <div className="absolute inset-0" style={{ opacity: 'var(--not-found-grid-opacity)' }}>
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(var(--not-found-grid-color) 1px, transparent 1px), linear-gradient(90deg, var(--not-found-grid-color) 1px, transparent 1px)',
          }}
        />
      </div>
      ...
      <div
        className="absolute bottom-16 right-8 z-10 h-16 w-16 -rotate-12 animate-[spin_15s_linear_infinite_reverse] rounded-lg sm:bottom-28 sm:right-28 sm:h-24 sm:w-24"
        style={{ border: '1px solid var(--not-found-floating-border)' }}
      />
      ...
      <div
        className="absolute bottom-1/4 right-1/3 z-10 h-14 w-14 rotate-[60deg] animate-[spin_25s_linear_infinite] rounded-lg sm:h-20 sm:w-20"
        style={{ border: '1px solid var(--not-found-floating-border)' }}
      />
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(circle at 50% 40%, var(--not-found-glow-color), transparent 60%)',
        }}
      />
      ...
      >
        ...
        >
          <span
            className="absolute inset-0 flex items-center justify-center font-mono text-[7rem] font-bold leading-none tracking-tighter blur-sm sm:text-[9rem] lg:text-[12rem]"
            style={{ color: 'var(--not-found-404-blur-color)' }}
          >
            404
          </span>
          <span
            className="relative font-mono text-[7rem] font-bold leading-none tracking-tighter text-accent sm:text-[9rem] lg:text-[12rem]"
            style={{ filter: 'var(--not-found-404-glow)' }}
          >
            404
          </span>
        </motion.div>
        ...
        >
          <p className="text-xs uppercase tracking-widest text-text-tertiary">{t('label')}</p>
          <h1 className="text-2xl font-semibold leading-tight tracking-tight text-accent sm:text-3xl">
            {t('heading')}
          </h1>
          <p
            className="text-base leading-relaxed sm:text-lg"
            style={{ color: 'var(--not-found-desc-color)' }}
          >
            {t('description')}
          </p>
        </motion.div>
