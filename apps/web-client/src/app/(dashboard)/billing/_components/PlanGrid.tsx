...
const getPlanIcon = (type: string) => {
    switch (type) {
        case SubscriptionPlan.STARTER: return <Rocket size={20} />;
        case SubscriptionPlan.GROWTH: return <TrendingUp size={20} />;
        case SubscriptionPlan.BUSINESS: return <Building2 size={20} />;
        case SubscriptionPlan.ENTERPRISE: return <Crown size={20} />;
  ...
  return (
    <div className="flex flex-wrap gap-6 justify-center">
      {sortedPlans.map((plan) => (
        <div
          key={plan.type}
          className={`flex-grow basis-[calc(100%)] sm:basis-[calc(50%-12px)] lg:basis-[calc(33.33%-16px)] 2xl:basis-[calc(20%-20px)] relative flex flex-col rounded-2xl border bg-white transition-all duration-300 hover:border-black/20 dark:bg-[#0A0A0A] dark:hover:border-white/20 ${plan.popular ? 'border-black shadow-lg ring-1 ring-black dark:border-white dark:ring-white' : 'border-black/5 dark:border-white/5'}`}
        >
          {plan.popular && (
          ...
          <div className="flex-1 p-6">
            <div className={`mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-black/5 text-black/80 transition-colors group-hover:bg-black group-hover:text-white dark:bg-white/5 dark:text-white/80 dark:group-hover:bg-white dark:group-hover:text-black`}>
              {getPlanIcon(plan.type)}
            </div>

            <h3 className="text-lg font-bold text-black dark:text-white">{plan.name}</h3>
            <p className="mt-1 min-h-[40px] text-sm text-black/50 dark:text-white/50">{plan.description}</p>
            
            <div className="mt-4 flex items-baseline gap-1">
              <span className="text-3xl font-bold font-geist-mono text-black dark:text-white">${plan.price.monthly}</span>
              <span className="text-sm font-medium text-black/40 dark:text-white/40">/{plan.price.currency}</span>
            </div>
            ...
            <div className="mt-6 space-y-3 border-t border-black/5 pt-6 dark:border-white/5">
                 ...
                 <div className="flex items-center gap-2 text-sm">
                    <span className="font-medium text-black/70 dark:text-white/70">
                         <strong className="text-black font-geist-mono dark:text-white">{plan.limits.monthlyCredits.toLocaleString()}</strong> créditos/mes
                    </span>
                 </div>
                 ...
                 <div className="flex items-center gap-2 text-sm text-black/70 dark:text-white/70">
                     <span>
                        {plan.limits.maxWorkflows === -1 ? 'Workflows ilimitados' : <><span className="font-geist-mono">{plan.limits.maxWorkflows}</span> Workflows</>}
                     </span>
                 </div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-black/70 dark:text-white/70">
                     <span>
                        {plan.limits.maxApiKeys === -1 ? 'API Keys ilimitadas' : <><span className="font-geist-mono">{plan.limits.maxApiKeys}</span> API Keys</>}
                     </span>
                 </div>
                 ...
                 <div className="flex items-center gap-2 text-sm text-black/70 dark:text-white/70">
                     <span>
                        {plan.limits.maxUsers === -1 ? 'Usuarios ilimitados' : <><span className="font-geist-mono">{plan.limits.maxUsers}</span> Usuarios</>}
                     </span>
                 </div>
            </div>
            ...
            <div className="mt-6 space-y-3">
              {plan.features.length > 0 && ( 
                <p className="text-xs font-bold uppercase tracking-wider text-black/30 dark:text-white/30">INCLUYE</p>
              )}
