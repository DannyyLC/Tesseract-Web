...
describe('TenantToolService', () => {
  ...
  describe('getWhatsappOutboundStatus', () => {
    ...
    it('excluye workflows que ya están enganchados a la tenant tool existente', async () => {
      ...
      expect(res.unlinkedWorkflows).toEqual([]);
      // Un solo número: no hay nada que elegir, no debe pedir default.
      expect(res.linkedWorkflowsNeedingDefault).toEqual([]);
    });

    it('un workflow ya enganchado con 2+ números pide default, con la preferencia guardada', async () => {
      mockPrismaService.whatsAppConfig.count.mockResolvedValue(2);
      mockPrismaService.tenantTool.findFirst.mockResolvedValue({
        id: 'tt-1',
        config: { default_by_workflow: { w1: 'c2' } },
        workflows: [{ id: 'w1' }],
      });
      mockPrismaService.whatsAppConfig.findMany.mockResolvedValue([
        {
      ...
      expect(res.linkedWorkflowsNeedingDefault).toEqual([
        {
          ...
          defaultWhatsappConfigId: 'c2',
        },
      ]);
    });

    it('ignora una preferencia guardada que ya no aplica a ninguno de los números actuales', async () => {
      mockPrismaService.whatsAppConfig.count.mockResolvedValue(2);
      mockPrismaService.tenantTool.findFirst.mockResolvedValue({
        id: 'tt-1',
        // "c-vieja" pudo ser un número que se desconectó del workflow después de guardarse.
        config: { default_by_workflow: { w1: 'c-vieja' } },
        workflows: [{ id: 'w1' }],
      });
      mockPrismaService.whatsAppConfig.findMany.mockResolvedValue([
        {
      ...
      expect(res.linkedWorkflowsNeedingDefault[0].defaultWhatsappConfigId).toBeNull();
    });
  });

  describe('setWhatsappOutboundDefault', () => {
    beforeEach(() => {
      mockPrismaService.toolCatalog.findFirst.mockResolvedValue({ id: 'cat-1' });
    });

    it('throws NotFoundException si el workflow no está enganchado a la tool', async () => {
      mockPrismaService.tenantTool.findFirst.mockResolvedValue(null);

      await expect(
        service.setWhatsappOutboundDefault('org-1', 'w1', 'c1'),
      ).rejects.toBeInstanceOf(NotFoundException);
    });

    it('throws NotFoundException si el número no le pertenece a ese workflow', async () => {
      mockPrismaService.tenantTool.findFirst.mockResolvedValue({ id: 'tt-1', config: {} });
      mockPrismaService.whatsAppConfig.findFirst.mockResolvedValue(null);

      await expect(
        service.setWhatsappOutboundDefault('org-1', 'w1', 'c-ajeno'),
      ).rejects.toBeInstanceOf(NotFoundException);
      expect(mockPrismaService.tenantTool.update).not.toHaveBeenCalled();
    });

    it('guarda la preferencia sin pisar el resto del config existente', async () => {
      mockPrismaService.tenantTool.findFirst.mockResolvedValue({
        id: 'tt-1',
        config: { whatsapp_config_id: ['c1', 'c2'], default_by_workflow: { w0: 'cX' } },
      });
      mockPrismaService.whatsAppConfig.findFirst.mockResolvedValue({ id: 'c2' });

      await service.setWhatsappOutboundDefault('org-1', 'w1', 'c2');

      expect(mockPrismaService.tenantTool.update).toHaveBeenCalledWith({
        where: { id: 'tt-1' },
        data: {
          config: {
            whatsapp_config_id: ['c1', 'c2'],
            default_by_workflow: { w0: 'cX', w1: 'c2' },
          },
        },
      });
    });

    it('con whatsappConfigId null borra la preferencia de ese workflow', async () => {
      mockPrismaService.tenantTool.findFirst.mockResolvedValue({
        id: 'tt-1',
        config: { default_by_workflow: { w1: 'c2', w2: 'c3' } },
      });

      await service.setWhatsappOutboundDefault('org-1', 'w1', null);

      expect(mockPrismaService.tenantTool.update).toHaveBeenCalledWith({
        where: { id: 'tt-1' },
        data: { config: { default_by_workflow: { w2: 'c3' } } },
      });
      // Borrar no necesita validar el número: no hay ninguno que validar.
      expect(mockPrismaService.whatsAppConfig.findFirst).not.toHaveBeenCalled();
    });
  });
