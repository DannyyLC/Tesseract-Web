...
describe('LlmModelsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [LlmModelsService, { provide: PrismaService, useValue: mockPrismaService }],
    }).compile();
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if model does not exist', async () => {
      ...
      await expect(service.update('invalid', { notes: 'updated' })).rejects.toThrow(
        NotFoundException,
      );
  ...
  describe('delete', () => {
    it('should set isActive to false and update effectiveTo', async () => {
      mockPrismaService.llmModel.findUnique.mockResolvedValue({
        id: 'm1',
  ...
  describe('calculateCost', () => {
    ...
    it('should throw error for invalid token counts', async () => {
      await expect(
        service.calculateCost('m1', { inputTokens: -10, outputTokens: 5, totalTokens: -5 }),
      ).rejects.toThrow('Token counts cannot be negative');
    });
  });
