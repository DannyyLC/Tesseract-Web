...
describe('ApiKeysService', () => {
  ...
  describe('create', () => {
    ...
    it('debería crear un API key exitosamente', async () => {
      ...
      expect(ApiKeyUtil.extractPrefix).toHaveBeenCalledWith(
        mockGeneratedApiKey,
      );
  ...
  describe('findOne', () => {
    ...
    it('debería lanzar NotFoundException si el API key no existe', async () => {
      ...
      // Act & Assert
      await expect(
        service.findOne(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow(NotFoundException);
      await expect(
        service.findOne(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow('API Key no encontrada');
    });
    ...
    it('debería lanzar ForbiddenException si el API key pertenece a otra organización', async () => {
      ...
      await expect(
        service.findOne(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow('No tienes permiso para ver esta API Key');
    });
  });
  ...
  describe('delete', () => {
    ...
    it('debería lanzar NotFoundException si el API key no existe', async () => {
      ...
      // Act & Assert
      await expect(
        service.delete(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debería lanzar ForbiddenException si el API key pertenece a otra organización', async () => {
      ...
      await expect(
        service.delete(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow('No tienes permiso para eliminar esta API Key');
    });
    ...
    it('debería lanzar ForbiddenException si el API key ya está eliminado', async () => {
      ...
      await expect(
        service.delete(mockOrganizationId, mockApiKeyId),
      ).rejects.toThrow('Esta API Key ya fue eliminada');
    });
  });
  ...
  describe('update', () => {
    ...
    it('debería lanzar BadRequestException si no se proporcionan campos para actualizar', async () => {
      ...
      // Act & Assert
      await expect(
        service.update(mockOrganizationId, mockApiKeyId, {}),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.update(mockOrganizationId, mockApiKeyId, {}),
    ...
    it('debería lanzar NotFoundException si el API key no existe', async () => {
      ...
      // Act & Assert
      await expect(
        service.update(mockOrganizationId, mockApiKeyId, updateDto),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debería lanzar ForbiddenException si el API key está eliminado', async () => {
      ...
      await expect(
        service.update(mockOrganizationId, mockApiKeyId, updateDto),
      ).rejects.toThrow('No puedes modificar una API Key eliminada');
    });
  });
});
