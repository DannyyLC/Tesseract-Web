...
export class BillingService {
    ...
    private async handleInvoicePaymentSucceeded(invoiceObject: Stripe.Invoice) {
        ...
        if (!organizationId) {
            this.logger.error(
                `No organizationId found in invoice metadata for invoice ${invoice.id}`,
            );
            return;
        }
        ...
        const plan = Object.values(SUBSCRIPTION_PLANS).find(
            (p: { priceUSD: number; credits: number; name: string; }) => p.priceUSD * 100 === amountPaidCents
        );
        ...
        if (plan) {
            ...
            this.logger.warn(`Invoice amount ${amountPaidCents} does not match any standard plan price. checking line items...`);
    ...
    async changePlan(organizationId: string, newPlan: SubscriptionPlan): Promise<void> {
        ...
        const priceId = this.configService.get(planConfig.priceIdEnvKey) || 'price_MISSING_CONFIG_' + newPlan;

        // 4. Retrieve Stripe Subscription to get the Item ID (required for update)
        const stripeSub = await this.stripeClient.stripe.subscriptions.retrieve(sub.stripeSubscriptionId);
    ...
    private async handleSubscriptionUpdated(subscription: Stripe.Subscription) {
        ...
        for (const [key, config] of Object.entries(SUBSCRIPTION_PLANS)) {
            ...
            if (priceId.includes(config.name) || priceId === this.configService.get(config.priceIdEnvKey)) {
        ...
        if (!planName) {
            ...
            if (sub.metadata?.plan) {
                ...
                this.logger.warn(`Could not determine plan for priceId ${priceId} in subscription ${sub.id}`);
                return;
            }
        }
    ...
    private async handleSubscriptionDeleted(subscription: Stripe.Subscription) {
        ...
        this.logger.log(`Subscription ${subscription.id} deleted. Org ${organizationId} downgraded to FREE.`);
    }
}
