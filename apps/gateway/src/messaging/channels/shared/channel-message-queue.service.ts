...
export type ChannelDrainedWindow<TEvent> = {
  messages: ChannelBufferedMessage<TEvent>[];
 ...
 * los tres juntos, en lugar de ejecutar el agente tres veces. Esta clase guarda los
 * mensajes de la ventana en Redis para que cualquier instancia pueda leerlos.
 *
 ...
 *
 * Cada canal aporta únicamente su prefijo de clave y el largo de su ventana; toda la
 * mecánica de Redis (buffer, drenado con RENAME, commit) es idéntica y vive aquí una
 * sola vez.
 *
 * @typeParam TEvent - Payload crudo del proveedor que se guarda por mensaje.
 */
export abstract class ChannelMessageQueueService<TEvent> {
  ...
  /** Namespace de las claves en Redis. Corto a propósito: `wa`, `ms`, … */
  protected abstract readonly keyPrefix: string;

  /** Silencio que hay que esperar antes de responder, en segundos. */
  protected abstract readonly windowSeconds: number;
  ...
  async bufferMessage(
    ...
    accountId: string,
    senderId: string,
    message: ChannelBufferedMessage<TEvent>,
  ): Promise<void> {
    const key = this.getConversationKey(organizationId, accountId, senderId);
  ...
  async drainWindow(
    ...
    senderId: string,
    windowId: string,
  ): Promise<ChannelDrainedWindow<TEvent>> {
    const key = this.getConversationKey(organizationId, accountId, senderId);
  ...
  buildWindowId(sendTimeMs: number = Date.now()): string {
    return String(Math.floor(sendTimeMs / 1000 / this.windowSeconds));
  }
  ...
  private parseMessages(values: unknown): ChannelBufferedMessage<TEvent>[] {
    ...
    return values
      .map((value) => {
        try {
          const parsed = JSON.parse(String(value)) as ChannelBufferedMessage<TEvent>;
          return parsed?.event ? parsed : null;
        } catch {
          return null;
        }
      })
      .filter((entry): entry is ChannelBufferedMessage<TEvent> => entry !== null);
  }
}
