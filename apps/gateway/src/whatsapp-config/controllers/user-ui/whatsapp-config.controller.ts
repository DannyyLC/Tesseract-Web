...
import {
    Body, Controller, Delete, Get, Headers, HttpStatus,
    Inject, Param, Patch, Post, Res, UseGuards,
} from '@nestjs/common';
...
import { CreateConfigDto, WhatsAppInboundEvent, CreateTemplateDto, UpdateTemplateDto, SendTemplateDto } from '../../dto';
...
export class WhatsappConfigController {
            ...
            if (yCloudApiKey && account.defaultWorkflowId) {
                if (isUnsupportedVideo) {
                    await this.whatsappConfigService.sendTextMessage(
                        yCloudApiKey, phoneNumber, userNumber,
    ...
    async createConfig(
        ...
        if (existingConfig) {
            apiResponse.setStatusCode(HttpStatus.BAD_REQUEST).setData(false)
        ...
        const response = await this.whatsappConfigService.createRecordAndgenerateWebhookSecret(
            currUser.organizationId, body.workflowId, body.phoneNumber,
        );
        if (response) {
            apiResponse.setStatusCode(HttpStatus.CREATED).setData(true)
                .setMessage('WhatsApp config created successfully');
            return res.status(HttpStatus.CREATED).json(apiResponse.build());
        }

        apiResponse.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR).setData(false)
    ...
    async deleteConfig(
        ...
        if (deleted) {
            apiResponse.setStatusCode(HttpStatus.OK).setData(true)
    ...
    async listConfigsByOrgAndWorkflow(
        ...
        const records = await this.whatsappConfigService.getConfigsByOrganizationAndWorkflow(
            currUser.organizationId, workflowId,
        );
        apiResponse.setStatusCode(HttpStatus.OK).setData(records).setMessage('WhatsApp configs retrieved');
        return res.status(HttpStatus.OK).json(apiResponse.build());
    }
    ...
    async sendTemplate(
        ...
        if (!template) {
            apiResponse.setStatusCode(HttpStatus.NOT_FOUND).setData(false).setMessage('Template not found');
            return res.status(HttpStatus.NOT_FOUND).json(apiResponse.build());
        }

        const config = await this.whatsappConfigService.getWhatsappConfigById(template.whatsAppConfigId);
        if (!config) {
            apiResponse.setStatusCode(HttpStatus.NOT_FOUND).setData(false).setMessage('WhatsApp config not found');
            return res.status(HttpStatus.NOT_FOUND).json(apiResponse.build());
        }
        ...
        await this.whatsappConfigService.sendTemplateMessage(
            apiKey, config.phoneNumber, body.to,
            template.name, template.language, body.variables,
        );
